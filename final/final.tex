...
When engineering the reward signal for a drafting sub-game where the encompassing game has only a single winner (such as Risk), it may be convenient to have $r_i(Z)$ approximate player $i$'s chances of winning from $Z$.  If we can provide a value $v_i$ representing the merit of player $i$'s picks, $A_i$, then
...
We use the game Risk as a testbed for our proposed drafting approaches.  Risk is a strategy board game for two to six players where the objective of the game is to occupy all 42 territories on the board.  In the standard rules, players first take turns selecting the territories until all have been chosen (the drafting sub-game).  After each player places their initial armies among their selected territories, players then take turns until only one player remains.  On a turn, in sequence a player may place reinforcements, conduct attacks on opposing territories, and make tactically maneuver armies amongst owned territories.  Attacks are resolved by dice rolls, where the number of dice is dependent on the number of attacking and defending armies.  Full rules can be found at \cite{WikipediaRisk}.

\section{Empirical Evaluation of Drafting}

In this section, we describe the drafting game we use called ``Fantasy Risk,'' a stand-alone drafting game that mimics the drafting sub-game of standard Risk.  We then compare our drafting approaches in this game, followed by an analysis of selfish versus non-selfish play.

\subsection{Fantasy Risk}

% Explain fantasy risk, plus give scoring table.
In Fantasy Risk, players take turns selecting from the 42 territories on the Risk board until all territories have been claimed, just as in the beginning of a standard game of Risk.  Then, the game immediately ends and the players receive rewards based on the outcome of the draft.  Playing Fantasy Risk allows us to better analyze our different drafting strategies without worrying about the dynamics of post-draft Risk.  We decided to run all of our experiments with three players, as this incorporates the multi-player aspect of drafting games and is arguably more strategically demanding compared to with more players because each player must make more selections per game.

\begin{table*}[t]
  \begin{center} 
   \caption{The continent points associated with Fantasy Risk.}
    ...
    \begin{tabular}{|c|c|c|c|c|c|c|}
    	...
    	\hline
    	\bf 5 & \cellcolor{black} & \cellcolor{black} & 0 & 19.3505 & 50.7666 & 23.6794 \\
    	\hline
    	\bf 6 & \cellcolor{black} & \cellcolor{black} & 29.796 & 24.8183 & 43.8472 & 19.3189 \\
    	\hline
    	\bf 7 & \cellcolor{black} & \cellcolor{black} & \cellcolor{black} & 24.0969 & 36.9278 & 15.6257 \\
    	\hline
    	\bf 8 & \cellcolor{black} & \cellcolor{black} & \cellcolor{black} & 36.1487 & \cellcolor{black} & 17.4338 \\
    	\hline
    	\bf 9 & \cellcolor{black} & \cellcolor{black} & \cellcolor{black} & 48.2005 & \cellcolor{black} & 13.8433 \\
    	\hline
    	\bf 10 & \cellcolor{black} & \cellcolor{black} & \cellcolor{black} & \cellcolor{black} & \cellcolor{black} & 10.2528 \\
    	\hline
    	\bf 11 & \cellcolor{black} & \cellcolor{black} & \cellcolor{black} & \cellcolor{black} & \cellcolor{black} & 6.6623 \\
    	\hline
    	\bf 12 & \cellcolor{black} & \cellcolor{black} & \cellcolor{black} & \cellcolor{black} & \cellcolor{black} & 3.0718 \\
    	\hline
    \end{tabular}
  \end{center}
\end{table*}
...
\begin{table}[t]
	\begin{center}
		\caption{Additional points associated with Fantasy Risk.} 
		\label{tab:MoreScoring}
		\begin{tabular}{|c|c|}
			\hline
			\textbf{Feature} & \textbf{Points} \\
			\hline
			First to pick & 13.3818 \\
			\hline
			Second to pick & 5.3459 \\
			\hline
...
We create end-game rewards for the players in Fantasy Risk by first accumulating a number of points $v_i$ for player $i$, $i=1,2,3$.  The points are determined by the number of territories owned in each continent, when the player picks in the turn order, the number of unique territories owned by other players which border owned territories (enemy neighbours), and the number of ordered pairs of owned territories which border each other (friendly neighbours).  Tables \ref{tab:ContScoring} and \ref{tab:MoreScoring} show the point values associated with each of these features.  

\subsection{Comparison of Algorithms}

% Mention how going first is an advantage and how we compensate for that

% Run experiments, graph results

\subsection{Selfish Play}

% Explain selfish play, run experiments, graph results

\section{Engineering Rewards For Playing Risk}

% Say what this section is all about

\subsection{Machine learning}

% Describe the linear regression we did in Weka plus linear extrapolation
