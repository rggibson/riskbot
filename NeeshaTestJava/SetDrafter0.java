...
import com.sillysoft.lux.agent.Logger;
 ...
 */
public class SetDrafter0 extends SmartAgentBase
{
    /**
...
//        System.out.println("0 " + board.getPlayerName(0));
//        System.out.println("1 " + board.getPlayerName(1));
//        System.out.println("2 " + board.getPlayerName(2));
//        board.getAgentName(player)
//        logger = new Logger(board);
        if (Logger.UseSelf()) {
        	ourNumber = 0;
        } else if (board.getAgentName(0).equalsIgnoreCase("SetDrafter0")) {
        	ourNumber = 0;
        } else if (board.getAgentName(1).equalsIgnoreCase("SetDrafter0")) {
        	ourNumber = 1;
        } else if (board.getAgentName(2).equalsIgnoreCase("SetDrafter0")) {
        	ourNumber = 2;
        }

        System.out.println("SD1 is " + ourNumber);
    ...
    {
    	int totalGames = Logger.getGamesplayed() + 1;
    	Logger.setGamesplayed(totalGames, ourNumber);
    ...
    public void attackPhase()
    {
    	if (!Logger.isPrinted()) {
	        ...
	        double[] eval =  evaluationFunction(draftState);
	        Logger.printEval(eval);
	        System.out.println("SD1 logger");
	        for (int i=0; i < eval.length; i++) {
	            System.out.println("player " + i + " has value " + eval[i]);
	        }
	        Logger.setPrinted(true);
    	}
    ...
    {
//    	int[] picks = {	29,14,4,7,41,31,25,9,15,35,11,17,30,34};
//    	int[] picks = {13, 16,17,2, 20, 23, 26, 27, 28 , 37, 5, 6, 8, 9};
//    	int[] picks = {0, 10, 11, 14, 15, 19, 24, 28, 29, 36, 41, 5, 6, 7};
//    	int[] picks = {0, 10, 11, 12, 13, 14, 21, 27, 28, 29, 37, 5, 7, 8};
//    	int[] picks = {10, 13, 19, 21, 22, 26, 31, 32, 34, 35, 36, 5, 6, 9};
    	
//    	System.out.println("SD1 picks " + picks[pick]);
        return Logger.getPlayerPick(ourNumber); 
    }

    /**
     * The depth in MaxN search where Monte Carlo roll outs take over.
     * TODO: rggibson - This should not be a fixed number.  It should really just
...
//        	System.out.println("player " + country[i].getOwner() + " owns country " + i);
//        }
